...
const ManeuverSandbox = () => {
  ...
  useEffect(() => {
    ...
    // Retry drawing until viewer is confirmed ready — viewer mount timing is unpredictable
    let attempts = 0;
    const tryDraw = () => {
      attempts++;
  ...
  const handleSimulate = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      // Remove old sim entities
      simTrackRef.current.forEach(e => { try { viewer.entities.remove(e); } catch {} });
      simTrackRef.current = [];
      ...
      // Draw simulated orbit in red
      segments.forEach((seg, idx) => {
        if (seg.length < 2) return;
        simTrackRef.current.push(viewer.entities.add({
          polyline: {
            positions: seg,
            width:     idx === 0 ? 3 : 2,
            material:  new Cesium.PolylineGlowMaterialProperty({
